			...
			<div class="main">
				...
				<table border="0" id="qrcode-table">
					<tbody>
						<tr>
							<td>
								<h2>Status: <span class="status">UNPAID</span></h2>
								<h2>Booking Code: <span style="font-weight: lighter">{{ $booking['code'] }}</span></h2>
								<h2>Booking Date: <span style="font-weight: lighter">{{ $schedule['from_date'] }}</span></h2>
							</td>
				...
				<table border="1" style="width: 100%;border-collapse: collapse;padding-top:15px;">
					<tbody>
						<table border="0" id="schedule-table">
							...
							<tbody>
								<tr>
									<td><b>Booking Date:</b> <br> {{ $schedule['from_date'] }} - {{ $schedule['to_date'] }}</td>
									<td><b>Total Day/s:</b> <br> {{ $schedule['total_days'] }}</td>
									<td><b>Total Passengers:</b> <br> {{ count($passengers) }}</td>
								</tr>
							</tbody>
						</table>
					</tbody>
								...
								<table border="0" id="pumpboat-table">
									...
									<tbody>
										...
										<tr>
											<td>Registration #</td>
											<td style="width: 2%">:</td>
											<td>{{ $pumpboat->reg_number }}</td>
										</tr>
										<tr>
											<td>Owner / Captain</td>
											<td style="width: 2%">:</td>
											<td>{{ $pumpboat->pumpboatOwner['first_name'] }} {{ $pumpboat->pumpboatOwner['last_name'] }} / {{ $pumpboat->pumpboatCaptain['first_name'] }} {{ $pumpboat->pumpboatCaptain['last_name'] }}</td>
										</tr>
										<tr>
											<td>Contact Details</td>
											<td style="width: 2%">:</td>
											<td>{{ $pumpboat->pumpboatOwner->contact_number }} | {{ $pumpboat->pumpboatOwner->email }}</td>
										</tr>
									</tbody>
								</table>
							</td>
